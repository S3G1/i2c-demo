...
uint16_t eCO = (data[0] << 8) | data[1];                                                            // eCo is two bytes so when we receive 8 
													bits we have to shift these two bits to the 
													left and join first byte and second
												        byte together. So we can have 16 bits 

HAL_UART_Transmit(&huart2,(uint8_t*)buffer,strlen(buffer), HAL_MAX_DELAY); 			// this function is used to send the data from stm32 to pc via UART protocol.